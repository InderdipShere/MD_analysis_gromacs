...
The Qn order parameter measures local crystalline ordering around each atom.
For each reference atom, neighbors within Rcut are identified and spherical 
harmonics of order n are computed to characterize the local environment.
The order parameter n can be specified per pair via --order.
...
def get_cli_args():
    ...
    parser.add_argument('-n', '--index_file', required=True, 
                        help='Index file (.ndx).')
    parser.add_argument('-o', '--output_file', default='Qn.xvg', 
                        help='Output file name for Qn results (default: Qn.xvg).')
    parser.add_argument('--order', type=int, nargs='+', default=None,
                        help='Steinhardt order parameter l for each pair (default: 6 for all pairs).')
...
def process_frame_q6(args, groups, is_self_pair, groups_mol, coords, box, atom_info, 
    ...
    """
    Process a single frame to compute Qn for all reference atoms.
    ...
    for pair_idx, (ref_name, sel_name) in enumerate(zip(args.ref, args.sel)):
        ...
        for ref_mol_id_global in sorted_all_ref_mols:
            ...
            q6 = compute_Q6_for_reference(neighbors, l=args.order[pair_idx])
...
def write_q6_output(args, q6_timeseries_by_pair, nn_timeseries_by_pair, all_neighbor_details):
    """Write Qn results to output file - multi-column format + enhanced detail."""
    ...
    logging.info(f"Qn time series written to {args.output_file}")
...
def main():
    """Main function to run Qn order parameter calculation."""
    # Record start time
    ...
    logging.info("=" * 70)
    logging.info("Qn STEINHARDT ORDER PARAMETER CALCULATION")
    logging.info("=" * 70)
