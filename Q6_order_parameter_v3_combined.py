#!/usr/bin/env python3
"""
Q6 STEINHARDT ORDER PARAMETER - V3 COMBINED
...
def get_cli_args():
    ...
    parser.add_argument('-o', '--output_file', default='Q6.xvg', help='Output file')
    ...
    for i, (ref_name, sel_name) in enumerate(zip(args_dict['ref'], args_dict['sel'])):
        ...
        for ref_atom_id in range(len(ref_pos)):
            ...
            if len(neighbor_indices) > 0:
                ...
                # VECTORIZED: Compute Q6 with batch harmonics
                q6 = compute_q6_vectorized(neighbor_deltas, l=6)
            else:
                neighbors = []
                q6 = np.nan
...
def write_q6_output(args, q6_timeseries_by_pair, nn_timeseries_by_pair, all_neighbor_details):
    """Write output - single multi-column Q6 file + separate NN file."""
    ...
    logging.info(f"Q6 time series written to {q6_file}")
...
def main():
    ...
    logging.info("=" * 70)
    logging.info("Q6 STEINHARDT - V3 COMBINED (Vectorized + Parallel)")
    logging.info("=" * 70)
