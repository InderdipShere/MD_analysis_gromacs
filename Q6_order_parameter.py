#!/usr/bin/env python3
"""
Calculates the Q6 Steinhardt order parameter from a GROMACS trajectory.

The Q6 order parameter measures local crystalline ordering around each atom.
...
def get_cli_args():
    ...
    parser.add_argument('-n', '--index_file', required=True, 
                        help='Index file (.ndx).')
    parser.add_argument('-o', '--output_file', default='Q6.xvg', 
                        help='Output file name for Q6 results (default: Q6.xvg).')
...
def compute_Q6_for_reference(neighbors_data, l=6):
    """
    Compute Q6 order parameter for a single reference atom.
    ...
    Returns:
        Q6 value (float), or np.nan if no neighbors
    """
...
def calculate_q6_trajectory(args, groups, is_self_pair, groups_mol, box_init):
    """
    Calculate Q6 order parameter for each frame (per-pair tracking).
    """
    npairs = len(args.ref)
...
def process_frame_q6(args, groups, is_self_pair, groups_mol, coords, box, atom_info, 
    ...
    """
    Process a single frame to compute Q6 for all reference atoms.
    ...
    for pair_idx, (ref_name, sel_name) in enumerate(zip(args.ref, args.sel)):
        ...
        for ref_mol_id_global in sorted_all_ref_mols:
            ...
            # Compute Q6 for this reference molecule
            q6 = compute_Q6_for_reference(neighbors, l=6)
...
def write_q6_output(args, q6_timeseries_by_pair, nn_timeseries_by_pair, all_neighbor_details):
    """Write Q6 results to output file - multi-column format + enhanced detail."""
    ...
    # Write Q6 file with multiple columns (one per pair)
    ...
    logging.info(f"Q6 time series written to {args.output_file}")
    ...
    for pair_idx in range(npairs):
        ...
        logging.info(f"\n{pair_name}:")
        logging.info(f"  Mean Q6:     {np.nanmean(q6_data):.6f}")
        logging.info(f"  Std Dev Q6:  {np.nanstd(q6_data):.6f}")
        logging.info(f"  Min Q6:      {np.nanmin(q6_data):.6f}")
        logging.info(f"  Max Q6:      {np.nanmax(q6_data):.6f}")
...
def main():
    """Main function to run Q6 order parameter calculation."""
    # Record start time
    ...
    logging.info("=" * 70)
    logging.info("Q6 STEINHARDT ORDER PARAMETER CALCULATION")
    logging.info("=" * 70)
