#!/usr/bin/env python3
"""
Calculates the Q2 Steinhardt order parameter from a GROMACS trajectory.

The Q2 order parameter measures local crystalline ordering around each atom.
...
def get_cli_args():
    ...
    parser.add_argument('-n', '--index_file', required=True, 
                        help='Index file (.ndx).')
    parser.add_argument('-o', '--output_file', default='Q2.xvg', 
                        help='Output file name for Q2 results (default: Q2.xvg).')
...
def compute_spherical_harmonics(theta, phi, l=2):
    """
...
def compute_Q2_for_reference(neighbors_data, l=2):
    """
    Compute Q2 order parameter for a single reference atom.
    ...
    Returns:
        Q2 value (float), or np.nan if no neighbors
    """
...
def calculate_q6_trajectory(args, groups, is_self_pair, groups_mol, box_init):
    """
    Calculate Q2 order parameter for each frame (per-pair tracking).
    """
    npairs = len(args.ref)
...
def process_frame_q6(args, groups, is_self_pair, groups_mol, coords, box, atom_info, 
    ...
    """
    Process a single frame to compute Q2 for all reference atoms.
    ...
    for pair_idx, (ref_name, sel_name) in enumerate(zip(args.ref, args.sel)):
        ...
        for ref_mol_id_global in sorted_all_ref_mols:
            ...
            # Compute Q2 for this reference molecule
            q6 = compute_Q2_for_reference(neighbors, l=2)
...
def write_q6_output(args, q6_timeseries_by_pair, nn_timeseries_by_pair, all_neighbor_details):
    """Write Q2 results to output file - multi-column format + enhanced detail."""
    ...
    # Write Q2 file with multiple columns (one per pair)
    ...
    logging.info(f"Q2 time series written to {args.output_file}")
    ...
    for pair_idx in range(npairs):
        ...
        logging.info(f"\n{pair_name}:")
        logging.info(f"  Mean Q2:     {np.nanmean(q6_data):.6f}")
        logging.info(f"  Std Dev Q2:  {np.nanstd(q6_data):.6f}")
        logging.info(f"  Min Q2:      {np.nanmin(q6_data):.6f}")
        logging.info(f"  Max Q2:      {np.nanmax(q6_data):.6f}")
...
def main():
    """Main function to run Q2 order parameter calculation."""
    # Record start time
    ...
    logging.info("=" * 70)
    logging.info("Q2 STEINHARDT ORDER PARAMETER CALCULATION")
    logging.info("=" * 70)
